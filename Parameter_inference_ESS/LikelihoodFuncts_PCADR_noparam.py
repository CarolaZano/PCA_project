...
def P_k_NL_nDGP(GR_pk2D_obj,cosmo, MGparams, k, a):
    """
    input k (array) -> wavevector, units 1/Mpc
    input a (float) -> scale factor (1/(1+z))and
    ...
    return pkratio_nDGP*Pk_ccltest

# NL matter power spectra in fR
def P_k_NL_fR(GR_pk2D_obj,cosmo, MGparams, k, a):
    """
    input k (array) -> wavevector, units 1/Mpc
...
def E(cosmoMCMCStep, a):
    Omg_r = cosmoMCMCStep["Omega_g"] * 2.09982058739
    Omg_m = cosmoMCMCStep["Omega_m"]
    Omg_lambda = 1 - Omg_m - Omg_r

    inv_a3 = 1 / a**3
    inv_a4 = 1 / a**4

    return np.sqrt(Omg_m * inv_a3 + Omg_r * inv_a4 + Omg_lambda)
...
def P_k_nDGP_lin(GR_pk2D_obj,cosmo, MGparams, k, a):
    """
    input k (array) -> wavevector, units 1/Mpc
    input a (float) -> scale factor (1/(1+z))
    input cosmo (cosmology object) -> Cosmology object from CCL
...
def mu_fR(fR0, cosmo, k, a):
    ...
    f0Pi = f0*(k/a)**2 + Zi**3/2/(3*cosmo["Omega_m"] - 4)**2
    return 1 + (k/a)**2/3 * f0/f0Pi

"""Linear matter power spectra f(R)"""
def solverGrowth_fR(y, a, cosmo, MGparams, k):
    E_val = E(cosmo, a)
    D , a3EdDda = y

    H0rc, fR0, n, mu, Sigma = MGparams
    mu = mu_fR(fR0, cosmo, k, a)
    
    ydot = [a3EdDda / (E_val*a**3), 3*cosmo["Omega_m"]*D*(mu)/(2*E_val*a**2)]
    return ydot
    
def P_k_fR_lin(GR_pk2D_obj,interp_fR_Pk,cosmo, MGparams, k, a):
    """
    input k (array) -> wavevector, units 1/Mpc
    input a (float) -> scale factor (1/(1+z))
    input cosmo (cosmology object) -> Cosmology object from CCL
    ...
    # Create a 2D grid of (k, a) for evaluation
    k_mesh, a_mesh = np.meshgrid(k, a, indexing='ij')
    points = np.stack([k_mesh.ravel(), a_mesh.ravel()], axis=-1)
    
    # Evaluate interpolation at (k, a)
    interp_values = interp_fR_Pk(points)
    
    # Multiply interpolated values by Pk_GR
    Pk_modified = Pk_GR * interp_values
...
ESS_C_Boost_loaded = np.load('parameters/Boost.npy')

z_k_arr_loaded = np.loadtxt('parameters/z_k.txt')
...
interpolator_ESS_C_mu_funct = scipy.interpolate.CubicSpline(np.loadtxt("parameters/ESS_run_ESS_force.txt").T[0], np.loadtxt("parameters/ESS_run_ESS_force.txt").T[2]+1)
...
def P_k_musigma(GR_pk2D_obj,cosmoMCMCStep, MGparams, k, a):
...
###################################################################
def sigma_8_musigma(GR_pk2D_obj,cosmo, MGparams, a_array):
...
def sigma_8_fR(GR_pk2D_obj,interp_fR_Pk,cosmo, MGparams, a_array):
    k_val = np.logspace(-4, 3, 3000)
    sigma_8_vals = []
...
def fsigma8_musigma(GR_pk2D_obj,cosmoMCMCStep, MGparams, a):
    ...
    k_val = np.logspace(-4,3,3000)
    return f_musigma * sigma_8_musigma(GR_pk2D_obj,cosmoMCMCStep, MGparams, a)

def fsigma8_nDGP(GR_pk2D_obj,cosmoMCMCStep, MGparams, a):
    ...
    k_val = np.logspace(-4,3,3000)
    return f_nDGP * sigma_8_nDGP(GR_pk2D_obj,cosmoMCMCStep, MGparams, a)

## Note: only works if we assume mu is approximately independent of k in f(R) !!!!
def solverGrowth_fR_const(y,a,cosmo, MGparams):
    E_val = E(cosmo, a)
    D , a3EdDda = y
    H0rc, fR0, n, mu, Sigma = MGparams
...
def fsigma8_fR(GR_pk2D_obj,interp_fR_Pk,cosmoMCMCStep, MGparams, a):
    ...
    a_solver = np.linspace(1/50,1,100)
    Soln = odeint(solverGrowth_fR_const, [a_solver[0], (E(cosmoMCMCStep, a_solver[0])*a_solver[0]**3)], a_solver, \
                  args=(cosmoMCMCStep,MGparams), mxstep=int(1e4))
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Pk_2D object ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def Get_Pk2D_obj(GR_pk2D_obj, interp_fR_Pk,cosmo, MGParams,linear=False,gravity_model="nDGP"):
    """
    ...
    def pk_func_nDGP_NL(k, a):
        ...
        pk_lin_start = P_k_nDGP_lin(GR_pk2D_obj, cosmo, MGParams, k[:idx_min], a)
        pk_nl_mid = P_k_NL_nDGP(GR_pk2D_obj,cosmo, MGParams, k_allowed, a)
        pk_nl_end = GR_pk2D_obj.__call__(k[idx_max:], a)
    ...
    def pk_func_fR_NL(k, a):        
        ...
        if a < 0.3333:
            return GR_pk2D_obj.__call__(k=k, a=a)
        ...
        interp_funct_fR = scipy.interpolate.PchipInterpolator([1e-3,k[idx_min]], \
                [1.0,P_k_NL_fR(GR_pk2D_obj,cosmo, MGParams, k[idx_min], a)[0]/GR_pk2D_obj.__call__(k[idx_min],a=a)])

        pk_lin_start = interp_funct_fR(k[:idx_min]) * GR_pk2D_obj.__call__(k=k[:idx_min], a=a)
        pk_nl_mid = P_k_NL_fR(GR_pk2D_obj,cosmo, MGParams, k_allowed, a)
        pk_nl_end = GR_pk2D_obj.__call__(k=k[idx_max:], a=a)
    ...
    def pk_func_nDGP_lin(k, a):
        ...
        # condition on z
        return P_k_nDGP_lin(GR_pk2D_obj,cosmo, MGParams, k, a)
    ...
    def pk_func_muSigma_lin(k, a):
        # condition on z
        return P_k_musigma(GR_pk2D_obj,cosmo, MGParams, k, a)
...
def Get_Pk2D_obj_delk_musigma(GR_pk2D_obj,cosmo, MGParams):
   
    ########### Functions for linear matter power spectrum multiplied by Sigma ###########        
    def pk_funcSigma_muSigma_lin(k, a):
        return sigma_lin_param(MGParams, cosmo,a)*P_k_musigma(GR_pk2D_obj,cosmo, MGParams, k, a)

    return ccl.pk2d.Pk2D.from_function(pkfunc=pk_funcSigma_muSigma_lin, is_logp=False)


def Get_Pk2D_obj_kk_musigma(GR_pk2D_obj,cosmo, MGParams):
   
    ########### Functions for linear matter power spectrum multiplied by Sigma**2 ###########        
    def pk_funcSigma2_muSigma_lin(k, a):
        return sigma_lin_param(MGParams, cosmo, a)**2 * P_k_musigma(GR_pk2D_obj,cosmo, MGParams, k, a)
...
def f_frac_err(GR_pk2D_obj,cosmo, MGparams,b, zeff, nbar, V):
    ...
    invcov = Pobs_covinv(GR_pk2D_obj,cosmo, MGparams,b, k, mu, zeff, nbar)
    ...
    f_sigma8_val = fsigma8_musigma(GR_pk2D_obj,cosmo, [0,0,0,0,0], [1./(1. + zeff),1./(1. + zeff)])[0]
    f_fid = f_sigma8_val/sigma_8_musigma(GR_pk2D_obj,cosmo, [0,0,0,0,0], [1./(1. + zeff),1./(1. + zeff)])[0]
    
    frac_err_f = err_f / f_fid
    
    return frac_err_f, f_sigma8_val
    
def diff_P_f(GR_pk2D_obj,cosmo, MGparams,b, k, mu, zeff):
    ...
    Pklin = ccl.linear_matter_power(cosmo, k=k,  a=1./ (1. + zeff))
    
    # Get the derivative at each mu / k
    f = fsigma8_musigma(GR_pk2D_obj,cosmo, [0,0,0,0,0], [1./(1. + zeff),1./(1. + zeff)])[0]/sigma_8_musigma(GR_pk2D_obj,cosmo, [0,0,0,0,0], [1./(1. + zeff),1./(1. + zeff)])[0]
    dPdf = [2. * (b + mu[mi]**2*f) * mu[mi]**2 * Pklin  for mi in range(len(mu))]
	
    return dPdf
    
def Pobs_covinv(GR_pk2D_obj,cosmo, MGparams,b, k, mu, zeff, nbar):
...
### Get OWLS sims P(k) to use for baryonic cuts ###
...
"""Define functions for OWLS P(k) to apply baryonic cuts"""
def Get_Pk2D_obj_OWLSAGN(cosmo, MGParams,linear=False,gravity_model="GR"):
...
def loglikelihood(Data, cosmo, MGparams, L_ch_inv, Bias_distribution, data_fsigma8):
    ...
    """
    # for nDGP PCA tests only: nDGP-1 GR-2 f(R)-3
    B_data =np.array([B3])
    M_data =np.array([M3])
    """
