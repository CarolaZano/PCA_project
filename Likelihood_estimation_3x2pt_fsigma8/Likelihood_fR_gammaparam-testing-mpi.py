...
filename = mcmc_dir + "/mcmc_fR_progress.h5"
...
with MPIPool() as pool:
    ...
    np.savetxt(mcmc_dir + "/updates_mcmc.txt", ["Multiprocessing took {0:.1f} seconds".format(multi_time)], fmt="%s")
...
open(mcmc_dir + "/updates_mcmc.txt", "a").write(["ratio convergence (want < 1) ={0:.2f}".format(max_tau_ratio)], fmt="%s")
open(mcmc_dir + "/updates_mcmc.txt", "a").write(str(sampler_PCA.chain.shape))

open(mcmc_dir + "/updates_mcmc.txt", "a").write("script ran successfully")
