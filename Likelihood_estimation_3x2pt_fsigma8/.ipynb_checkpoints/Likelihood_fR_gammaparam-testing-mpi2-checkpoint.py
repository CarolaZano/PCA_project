...
# MGCAMB
MODULE_PATH = "/home/dc-zano2/MGCAMB/camb/__init__.py"
MODULE_NAME = "MGCAMB"
import importlib
...
mcmc_dir = "/home/dc-zano2/PCADR_project/mcmc"

#### Get Planck priors #####
sampler_Planck_arr = np.load("/home/dc-zano2/PCADR_project/Prior_Planck_arr.npy")
...
def log_prior(theta):
    ...
    cosmo =  ccl.Cosmology(Omega_c = Omega_c,
                      ...
                      A_s = A_s1e9*1e-9)
    #flat priors
    if not (0.28 < Omega_c + wb/h**2 < 0.36  and 0.6083 < ccl.sigma8(cosmo) < 1.014\
            and 0.0 < mu0 < 1.0 and -0.7 < Sigma0 < 0.7 \
...
#os.system(command)
## Collect the data
npzfile = np.load("/home/dc-zano2/PCADR_project/Data_storage.npz")
...
# Set up parameters
n_steps = 3000  # Total steps you want to run
nwalkers = cpu_count()  # Number of walkers (can be different if needed)
nwrite = 2  # Number of steps after which progress is written to file
...
with MPIPool() as pool:
    ...
    # Run the MCMC with periodic checkpointing
    for i, result in enumerate(sampler_PCA.sample(pos, iterations=n_steps, progress=True)):
        if (i + 1) % nwrite == 0:  # Save every `nwrite` steps
            backend.save()  # Save the current progress to the HDF5 file
            print(f"Checkpoint saved at step {i + 1}")

    end = time.time()
    multi_time = end - start

    # Save the total run time to a text file
    np.savetxt(mcmc_dir + "/updates_mcmc.txt", [f"Multiprocessing took {multi_time:.1f} seconds"], fmt="%s")

    # Save additional convergence information
    tau = sampler_PCA.get_autocorr_time(tol=0)
    np.save(mcmc_dir + "/fR_GammaParam_PCAcut.npy", sampler_PCA.chain)
    max_tau_ratio = np.max(tau * 100 / sampler_PCA.iteration)
    with open(mcmc_dir + "/updates_mcmc.txt", "a") as f:
        f.write(f"ratio convergence (want < 1) = {max_tau_ratio:.2f}\n")
        f.write(f"Chain shape: {sampler_PCA.chain.shape}\n")
